...
function uploadConfig() {
    ...
    // Validações
    if (!configType) {
        showUploadStatus('Selecione o tipo de configuração', 'warning');
        return;
    }
    
    if (!configFile) {
        showUploadStatus('Selecione um arquivo JSON', 'warning');
        return;
    }
    ...
    .then(data => {
        if (data.success) {
            ...
            showUploadStatus(`Erro: ${data.error}`, 'error');
        }
    })
    .catch(error => {
...
function downloadConfig(configType) {
    const typeNames = {
        ...
        'mqtt': 'MQTT Client',
        'network': 'Network',
        'main': 'Main (General Configuration)'
    };
    
    const url = `/api/config/download/${configType}`;
    const fileName = `${configType}_config.json`;
    console.log(`[downloadConfig] Iniciando download: type=${configType}, url=${url}`);
    
    fetch(url)
    .then(response => {
        console.log(`[downloadConfig] Resposta recebida: status=${response.status}, ok=${response.ok}`);
        const contentType = response.headers.get('Content-Type') || '';
        if (!response.ok) {
            throw new Error(`HTTP ${response.status}`);
        }
        // Detecta se veio página de acesso negado ao invés do JSON
        if (contentType.includes('text/html')) {
            return response.text().then(text => {
                console.warn('[downloadConfig] Conteúdo HTML recebido em vez de JSON. Possível falta de permissão.', text.substring(0,200));
    ...
    .then(blob => {
        ...
        console.log(`[downloadConfig] Download concluído: ${fileName}`);
        ...
        fileInput.addEventListener('change', function(e) {
            ...
            if (file) {
                if (!file.name.toLowerCase().endsWith('.json')) {
                    showUploadStatus('Apenas arquivos .json são aceitos', 'warning');
                    this.value = '';
                } else if (file.size > 10240) {
                    showUploadStatus('Arquivo muito grande. Máximo: 10KB', 'warning');
                    this.value = '';
                } else {
...
// Garantir que as funções principais fiquem disponíveis no escopo global
// para evitar ReferenceError caso algum wrapper futuro seja aplicado.
